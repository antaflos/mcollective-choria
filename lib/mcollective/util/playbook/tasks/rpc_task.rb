      ...
      class Tasks
        class RpcTask
          ...
          def validate_configuration!
            raise("Nodes need to be supplied, refusing to run against all nodes") if @nodes.empty?
          end
          ...
          def log_success(stats)
            Log.debug("Successful request %s for %s#%s in %0.2fs against %d node(s)" % [
              stats.requestid,
              @agent,
              @action,
              stats.totaltime,
          ...
          def log_summarize(stats)
            ...
            unless summary.empty?
              if @description
                _d = "%s (%s#%s)" % [@description, @agent, @action]
              else
                _d = "%s#%s" % [@agent, @action]
              end

              Log.info("Summary for %s: %s" % [_d, summary.inspect])
            end
          end
          ...
          def log_failure(stats, replies)
            ...
            Log.warn("Failed request %s for %s#%s in %0.2fs. %d successful node(s)" % [
              stats.requestid,
              @agent,
              @action,
              stats.totaltime,
            ...
            if stats.failcount > 0
              replies.each do |reply|
                if reply.results[:statuscode] > 0
                  Log.warn("Failed result from %s: %s" % [
                    reply.results[:sender],
                    reply.results[:statusmsg]
                  ])
                end
              end
            end
          ...
          def run
            ...
            begin
              ...
              client.send(@action, @properties) do |r, s|
                replies << s
                log_reply(s)
              end
